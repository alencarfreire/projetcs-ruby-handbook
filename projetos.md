...
└── projects/                     app que roda
    └── 01-http-api/
        ├── README.md             o que constrói, como rodar, curls
        └── server.rb             API completa (stdlib)
```
...
Código: [`projects/01-http-api`](/projects/01-http-api/) · [server.rb no GitHub](https://github.com/alencarfreire/projetcs-ruby-handbook/blob/main/projects/01-http-api/server.rb)
...
`server.rb` — TCPServer, rotas na mão, JSON, `@tasks` + `@next_id`. Um arquivo de propósito. Não é Rails.
